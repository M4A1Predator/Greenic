...
CREATE TABLE `greenic_article` (
  ...
  `article_cover_image` varchar(512) DEFAULT NULL,
  ...
  KEY `fk_greenic_article_greenic_status1_idx` (`article_status_id`),
  ...
  CONSTRAINT `fk_greenic_article_greenic_status1` FOREIGN KEY (`article_status_id`) REFERENCES `greenic_status` (`status_id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Table structure for table `greenic_breed`
--

DROP TABLE IF EXISTS `greenic_breed`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `greenic_breed` (
  `breed_id` int(11) NOT NULL AUTO_INCREMENT,
  `breed_name` varchar(256) NOT NULL,
  `breed_category_id` int(11) NOT NULL,
  `breed_creator_id` int(11) DEFAULT NULL,
  PRIMARY KEY (`breed_id`),
  KEY `fk_greenic_breed_greenic_category1_idx` (`breed_category_id`),
  KEY `fk_greenic_breed_greenic_member1_idx` (`breed_creator_id`),
  CONSTRAINT `fk_greenic_breed_greenic_category1` FOREIGN KEY (`breed_category_id`) REFERENCES `greenic_category` (`category_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `fk_greenic_breed_greenic_member1` FOREIGN KEY (`breed_creator_id`) REFERENCES `greenic_member` (`member_id`) ON DELETE SET NULL ON UPDATE SET NULL
...
CREATE TABLE `greenic_category` (
  ...
  `category_creator_id` int(11) DEFAULT NULL,
  PRIMARY KEY (`category_id`),
  KEY `fk_greenic_category_greenic_project_type1_idx` (`category_project_type_id`),
  KEY `fk_greenic_category_greenic_member1_idx` (`category_creator_id`),
  CONSTRAINT `fk_greenic_category_greenic_member1` FOREIGN KEY (`category_creator_id`) REFERENCES `greenic_member` (`member_id`) ON DELETE SET NULL ON UPDATE SET NULL,
...
/*!50001 CREATE VIEW `greenic_category_view` AS SELECT 
 ...
 1 AS `category_datetime`,
 1 AS `category_project_type_id`,
 1 AS `category_creator_id`,
...
--
-- Temporary view structure for view `greenic_member_public_view`
--

DROP TABLE IF EXISTS `greenic_member_public_view`;
/*!50001 DROP VIEW IF EXISTS `greenic_member_public_view`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `greenic_member_public_view` AS SELECT 
 1 AS `member_id`,
 1 AS `member_email`,
 1 AS `member_firstname`,
 1 AS `member_lastname`,
...
--
-- Table structure for table `greenic_product_status`
--

DROP TABLE IF EXISTS `greenic_product_status`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `greenic_product_status` (
  `product_status_id` int(11) NOT NULL AUTO_INCREMENT,
  `product_status_name` varchar(50) NOT NULL,
  `product_status_name_th` varchar(150) NOT NULL,
  `product_status_order` int(11) DEFAULT '1',
...
CREATE TABLE `greenic_project` (
  ...
  KEY `fk_greenic_project_greenic_breed1_idx` (`project_breed_id`),
  KEY `fk_greenic_project_greenic_product_status1_idx` (`project_product_status_id`),
  CONSTRAINT `fk_greenic_project_greenic_breed1` FOREIGN KEY (`project_breed_id`) REFERENCES `greenic_breed` (`breed_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ...
  CONSTRAINT `fk_greenic_project_greenic_product_status1` FOREIGN KEY (`project_product_status_id`) REFERENCES `greenic_product_status` (`product_status_id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
/*!50001 CREATE VIEW `greenic_project_view` AS SELECT 
 ...
 1 AS `breed_id`,
 1 AS `breed_name`,
 1 AS `breed_category_id`,
 1 AS `breed_creator_id`,
 1 AS `farm_id`,
 1 AS `farm_name`,
 1 AS `farm_province`,
 1 AS `farm_district`,
 ...
 1 AS `unit_name`,
 1 AS `unit_order`,
 1 AS `unit_creator_id`,
 1 AS `status_id`,
 1 AS `status_name`,
 1 AS `product_status_id`,
 1 AS `product_status_name`,
 1 AS `product_status_name_th`,
 1 AS `product_status_order`*/;
SET character_set_client = @saved_cs_client;
...
CREATE TABLE `greenic_review` (
  ...
  `review_buyamount` float DEFAULT NULL,
...
CREATE TABLE `greenic_sub_district` (
  ...
  CONSTRAINT `fk_greenic_sub_district_greenic_district1` FOREIGN KEY (`sub_district_district_id`) REFERENCES `greenic_district` (`district_id`) ON DELETE CASCADE ON UPDATE NO ACTION,
...
--
-- Table structure for table `greenic_webdata`
--

DROP TABLE IF EXISTS `greenic_webdata`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `greenic_webdata` (
  `webdata_id` int(11) NOT NULL AUTO_INCREMENT,
  `webdata_name` varchar(300) NOT NULL,
  `webdata_value` text,
...
/*!50001 VIEW `greenic_category_view` AS select `category`.`category_id` AS `category_id`,`category`.`category_name` AS `category_name`,`category`.`category_description` AS `category_description`,`category`.`category_datetime` AS `category_datetime`,`category`.`category_project_type_id` AS `category_project_type_id`,`category`.`category_creator_id` AS `category_creator_id`,`pt`.`project_type_id` AS `project_type_id`,`pt`.`project_type_name` AS `project_type_name` from (`greenic_category` `category` join `greenic_project_type` `pt` on((`pt`.`project_type_id` = `category`.`category_project_type_id`))) */;
...
--
-- Final view structure for view `greenic_member_public_view`
--
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `greenic_project_view` AS select `project`.`project_id` AS `project_id`,`project`.`project_name` AS `project_name`,`project`.`project_detail` AS `project_detail`,`project`.`project_location` AS `project_location`,`project`.`project_time` AS `project_time`,`project`.`project_startdate` AS `project_startdate`,`project`.`project_enddate` AS `project_enddate`,`project`.`project_selldate` AS `project_selldate`,`project`.`project_quantity` AS `project_quantity`,`project`.`project_ppu` AS `project_ppu`,`project`.`project_lowest_order` AS `project_lowest_order`,`project`.`project_cover_image_path` AS `project_cover_image_path`,`project`.`project_view` AS `project_view`,`project`.`project_category_id` AS `project_category_id`,`project`.`project_breed_id` AS `project_breed_id`,`project`.`project_farm_id` AS `project_farm_id`,`project`.`project_status_id` AS `project_status_id`,`project`.`project_unit_id` AS `project_unit_id`,`project`.`project_product_status_id` AS `project_product_status_id`,`category`.`category_id` AS `category_id`,`category`.`category_name` AS `category_name`,`category`.`category_description` AS `category_description`,`category`.`category_project_type_id` AS `category_project_type_id`,`category`.`category_creator_id` AS `category_creator_id`,`category`.`project_type_id` AS `project_type_id`,`category`.`project_type_name` AS `project_type_name`,`breed`.`breed_id` AS `breed_id`,`breed`.`breed_name` AS `breed_name`,`breed`.`breed_category_id` AS `breed_category_id`,`breed`.`breed_creator_id` AS `breed_creator_id`,`farm`.`farm_id` AS `farm_id`,`farm`.`farm_name` AS `farm_name`,`farm`.`farm_province` AS `farm_province`,`farm`.`farm_district` AS `farm_district`,`farm`.`farm_sub_district` AS `farm_sub_district`,`farm`.`farm_address` AS `farm_address`,`farm`.`farm_member_id` AS `farm_member_id`,`farm`.`farm_status_id` AS `farm_status_id`,`farm`.`member_id` AS `member_id`,`farm`.`member_email` AS `member_email`,`farm`.`member_firstname` AS `member_firstname`,`farm`.`member_lastname` AS `member_lastname`,`farm`.`member_password` AS `member_password`,`farm`.`member_img_path` AS `member_img_path`,`farm`.`member_address` AS `member_address`,`farm`.`member_province` AS `member_province`,`farm`.`member_district` AS `member_district`,`farm`.`member_sub_district` AS `member_sub_district`,`farm`.`member_token` AS `member_token`,`farm`.`member_regis_time` AS `member_regis_time`,`farm`.`member_verify_time` AS `member_verify_time`,`farm`.`member_type_id` AS `member_type_id`,`farm`.`member_status_id` AS `member_status_id`,`farm`.`member_status_name` AS `member_status_name`,`farm`.`member_type_name` AS `member_type_name`,`farm`.`farm_status_name` AS `farm_status_name`,`unit`.`unit_id` AS `unit_id`,`unit`.`unit_name` AS `unit_name`,`unit`.`unit_order` AS `unit_order`,`unit`.`unit_creator_id` AS `unit_creator_id`,`state`.`status_id` AS `status_id`,`state`.`status_name` AS `status_name`,`product_status`.`product_status_id` AS `product_status_id`,`product_status`.`product_status_name` AS `product_status_name`,`product_status`.`product_status_name_th` AS `product_status_name_th`,`product_status`.`product_status_order` AS `product_status_order` from ((((((`greenic_project` `project` join `greenic_category_view` `category` on((`category`.`category_id` = `project`.`project_category_id`))) left join `greenic_breed` `breed` on((`breed`.`breed_id` = `project`.`project_breed_id`))) join `greenic_farm_view` `farm` on((`farm`.`farm_id` = `project`.`project_farm_id`))) join `greenic_unit` `unit` on((`unit`.`unit_id` = `project`.`project_unit_id`))) join `greenic_status` `state` on((`state`.`status_id` = `project`.`project_status_id`))) join `greenic_product_status` `product_status` on((`product_status`.`product_status_id` = `project`.`project_product_status_id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2016-10-09 19:01:44
